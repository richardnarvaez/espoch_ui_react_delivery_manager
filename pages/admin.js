...
// Fragmentos
import OrderFragment from '../components/admin/1. OrdersFragment'
import Fragment2 from '../components/admin/2. StadisticsFragment'
import ProductFragment from '../components/admin/3. ProductsFragment'
import ProfileFragment from '../components/admin/4. ProfileFragment'
...
const user_data_test = {
	meAdmin: {
		_id: 'dasdjklmldiweusdnd',
		name: 'nombre prueba',
		email: 'prueba',
		business: '5f77eb55d68385e57a1d933f',
		status: true,
	},
	business: '-jasdShaShjda-2opqS',
}

const MainView = ({ item, user, business }) => {
	switch (item) {
		case 'f0':
...
const Admin = () => {
	...
	const [itemDrawerIndex, setItemDrawerIndex] = useState('f0')

	const bottomBarItemClick = (item, num) => {
		setItemDrawerIndex(item)
		setItemActive(num)
	}
	...
	return (
		<div style={{ position: 'relative', display: 'flex', height: '100vh' }}>
			...
			<div
				...
				<MainView
					item={itemDrawerIndex}
					user={user_data_test.meAdmin}
					business={user_data_test.business}
				/>
			</div>
		</div>
	)
